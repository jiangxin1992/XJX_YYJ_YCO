...
static NSString * const LanguageStrings[] = { @"English", @"简体中文" };
static NSString * const LanguageSaveKey = @"currentLanguageKey";

@implementation LanguageManager

+ (void)setLanguageToServer{
    YYUser *currentUser = [YYUser currentUser];
    if(currentUser){
        ELanguage nowLang = [LanguageManager isEnglishLanguage]?ELanguageEnglish:ELanguageChinese;
        [YYUserApi setLanguageToServer:nowLang andBlock:^(YYRspStatusAndMessage *rspStatusAndMessage, NSError *error) {
        }];
    }
}
