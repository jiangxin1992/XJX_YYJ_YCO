...
- (void)setFirstResponderView:(UIView *)firstResponderView
{
    ...
    if (!firstResponderView.inputAccessoryView) {
        ...
        if (dodgeView) {
            ...
            self.retractInputAccessoryView.frame = CGRectMake(0, 0, screenWidth, kMLInputDodgerRetractViewDefaultHeight);
...
- (void)updateInputViewDetailWithKeyboardNotification:(NSNotification *)notification
{
    ...
    if (SYSTEM_VERSION_LESS_THAN(@"8.0")){
        ...
        self.inputViewFrame = CGRectMake(self.inputViewFrame.origin.y, self.inputViewFrame.origin.x, self.inputViewFrame.size.height, self.inputViewFrame.size.width);
    }
    //NSLog(@"[UIScreen mainScreen].bounds.size.height  %f",screenHeight);
    if (self.inputViewFrame.origin.y>screenHeight) {
        CGRect adjustFrame = self.inputViewFrame;
        adjustFrame.origin.y = screenHeight-adjustFrame.size.height;
...
- (void)doDodgeWithAnimated:(BOOL)animated dodgeScrollView:(UIScrollView*)dodgeView
{
    ...
    if (self.lastFirstResponderViewForShowInputView) {
        ...
        //NSLog(@"keyboardOrginY %f,%f",keyboardOrginY,keyboardOrginY);
...
//        if (SYSTEM_VERSION_LESS_THAN(@"8.0")){
//            dodgeViewFrameBottomInWindow = CGRectGetMaxX(dodgeViewFrameInWindow);
//        }
        NSLog(@"dodgeViewFrameBottomInWindow %f,%f",dodgeViewFrameBottomInWindow,keyboardOrginY);
