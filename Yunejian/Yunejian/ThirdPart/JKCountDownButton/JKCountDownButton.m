...
//        _second--;
        if (_didChangeBlock)
        {
            NSString *showStr = _didChangeBlock(self,_second);
            self.titleLabel.text = showStr;
            [self setTitle:showStr forState:UIControlStateNormal];
            [self setTitle:showStr forState:UIControlStateDisabled];
        ...
        if ([_timer respondsToSelector:@selector(isValid)])
        {
            if ([_timer isValid])
            {
                ...
                else
                {
                    ...
                    [self setTitle:NSLocalizedString(@"重新获取",nil) forState:UIControlStateDisabled];
