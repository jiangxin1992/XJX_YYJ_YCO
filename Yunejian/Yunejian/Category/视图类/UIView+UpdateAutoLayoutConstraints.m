...
- (BOOL) setConstraintConstant:(CGFloat)constant forAttribute:(NSLayoutAttribute)attribute
{
    ...
    {
        [self addConstraint: [NSLayoutConstraint constraintWithItem:self attribute:attribute relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0f constant:constant]];
        return NO;
    }
}
...
- (NSLayoutConstraint*) constraintForAttribute:(NSLayoutAttribute)attribute
{
    ...
    {
        for (NSLayoutConstraint *constraint in fillteredArray) {
            if(constraint.class == [NSLayoutConstraint class]){
                return constraint;
            }
           // NSLog(@"%@",constraint.class);
        }
