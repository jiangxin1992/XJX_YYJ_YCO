...
-(void)PrepareData{
    ...
    _menuBtnsData = @[@[@"",NSLocalizedString(@"全部",nil),@"1"]
                      ...
                      ,@[@"",NSLocalizedString(@"发货中",nil),@"5"]
                      ,@[@"",NSLocalizedString(@"已发货",nil),@"6"]
                      ,@[@"",NSLocalizedString(@"已收货",nil),@"7"]
                      ,@[@"",NSLocalizedString(@"部分货款已收",nil),@"8",@""]
                      ,@[@"",NSLocalizedString(@"100%货款已收",nil),@"9"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([obj isKindOfClass:[YYOrderListItemModel class]]) {
        ...
        orderListItemModel.designerTransStatus = [NSNumber numberWithInt:YYOrderCode_NEGOTIATION];//
        orderListItemModel.buyerTransStatus = [NSNumber numberWithInt:YYOrderCode_NEGOTIATION];//
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([obj isKindOfClass:[YYOrderListItemModel class]]) {
        ...
        if (orderListItemModel
            ...
            && tranStatus != YYOrderCode_CANCELLED && tranStatus != YYOrderCode_CLOSED && tranStatus != YYOrderCode_CLOSE_REQ && tranStatus != YYOrderCode_DELIVERY && tranStatus != YYOrderCode_RECEIVED && [orderListItemModel.closeReqStatus integerValue] != -1) {
...
-(void)deliverWithListItemModel:(YYOrderListItemModel *)orderListItemModel WithIndexPath:(NSIndexPath *)indexPath{
    ...
    [createPackingListViewController setModifySuccess:^{
        //发货成功啦！咱们去对应的订单详情页吧
        //进去订单详情
        [ws showDetailOrderView:tmpCreatePackingListViewController.indexPath];
    }];
...
-(void)updateTransStatusWithListItemModel:(YYOrderListItemModel *)orderListItemModel indexPath:(NSIndexPath *)indexPath{
    ...
    NSInteger nextTransStatus = getOrderNextStatus(transStatus,[orderListItemModel.connectStatus integerValue]);
...
- (void)showDetailOrderView:(NSIndexPath *)indexPath{
    ...
    if ([obj isKindOfClass:[YYOrderListItemModel class]]) {
        ...
        orderDetailViewController.currentOrderConnStatus = YYOrderConnStatusUnconfirmed;
    }
