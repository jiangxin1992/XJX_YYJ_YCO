...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *buyerNameButtonWidthLayout;//185
...
- (void)PrepareUI{
    if([LanguageManager isEnglishLanguage]){
        _retailerWidthLayout.constant = 170;
        _addressWidthLayout.constant = 170;
    }else{
    ...
    _addressLayoutTopConstraints.constant = 235+70;// 125
...
- (void)updateUI{
    ...
    if (_currentYYOrderInfoModel) {
        ...
        if([NSString isNilOrEmpty:_currentYYOrderInfoModel.type]){
            [_orderTypeButton setTitle:NSLocalizedString(@"选择订单类型",nil)  forState:UIControlStateNormal];
        }else{
            if([_currentYYOrderInfoModel.type isEqualToString:@"BUYOUT"]){
                [_orderTypeButton setTitle:@"买断（Buy out）" forState:UIControlStateNormal];
            }else if([_currentYYOrderInfoModel.type isEqualToString:@"CONSIGNMENT"]){
                [_orderTypeButton setTitle:@"寄售（Consignment sale）" forState:UIControlStateNormal];
            }else{
                [_orderTypeButton setTitle:NSLocalizedString(@"选择订单类型",nil)  forState:UIControlStateNormal];
            }
        }
        ...
        if(_currentYYOrderInfoModel != nil && !hasInitBuyerNameLabel ){
            if (_currentYYOrderInfoModel.buyerName && [_currentYYOrderInfoModel.buyerName length] >0) {
                if(self.orderCreateBuyerNameButtonClicked){
                    ...
                    _buyerNameButtonWidthLayout.constant = MAX(341, buyerStrSize.width+30);
                    _buyerNameRightTipImg.hidden = NO;
                    ...
                    _buyerNameButtonWidthLayout.constant = MAX(341,nameTxtsize.width+30);
                }
            }else{
                _buyerNameButtonWidthLayout.constant = 341;
                if(self.orderCreateBuyerNameButtonClicked){
                    _buyerNameRightTipImg.hidden = NO;
...
- (IBAction)orderTypeButtonClicked:(id)sender {
    if(self.orderTypeButtonClicked){
        self.orderTypeButtonClicked(sender);
    }
}
