...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *logoImageLoayoutSoaceConstraints;
...
- (void)updataUI{  //记得清空假数据
    ...
    if(false && user.userType == kBuyerStorUserType){
        ...
        if(true && _currentYYOrderInfoModel && _currentOrderConnStatus >=-1){
            ...
            if(_currentOrderConnStatus == kOrderStatus || _currentOrderConnStatus == kOrderStatus3){
            ...
            if((_currentOrderConnStatus != kOrderStatus1)&&(orderStatus == kOrderCode_NEGOTIATION)){
    ...
    self.timeLab.text = [NSString stringWithFormat:NSLocalizedString(@"订单编号：%@     建立时间：%@     ",nil),tmpOrderCode,[formatter stringFromDate:date]];
