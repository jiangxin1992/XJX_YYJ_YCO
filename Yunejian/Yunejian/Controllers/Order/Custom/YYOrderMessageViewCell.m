...
-(void)updateUI{
    ...
    if([_msgInfoModel.autoCloseHoursRemains integerValue] > 0){
        ...
        [_orderCloseTimerLabel setTitle:[NSString stringWithFormat:NSLocalizedString(@"剩余%ld天%ld小时，交易将自动取消",nil),(long)day,(long)hours] forState:UIControlStateNormal];
        [_orderCloseTimerLabel setImage:[UIImage imageNamed:@"warn"] forState:UIControlStateNormal];
