...
-(void)updateUI{
    ...
    if( [_noteModel.payType integerValue] == 0){
        ...
        if(statusValue == 0){//支付状态(线下) 0:待确认 1：成功到账 2：已作废
            ...
            if(([_noteModel.tmpPercent integerValue] + [_noteModel.percent integerValue]) >100){
...
- (IBAction)btnClickHandler:(id)sender {
    return;
    if([_noteModel.payType integerValue] == 1){
        if(self.delegate){
        if(_detailView.hidden){
            [self.delegate btnClick:_indexPath.row section:_indexPath.section andParmas:@[@"show"]];
        }else{
            [self.delegate btnClick:_indexPath.row section:_indexPath.section andParmas:@[@"hide"]];
        }
        }
    }
}
