//
//  YYConnBuyerInfoContactViewController.m
//  Yunejian
//
...
@interface YYConnBuyerInfoContactViewController ()
...
@implementation YYConnBuyerInfoContactViewController
...
-(void)CreateNoDataView
{
    ...
    _noIntroductionDataView = (YYNoDataView *)addNoDataView_pad(self.view,[NSString stringWithFormat:@"%@|icon:notxt_icon",NSLocalizedString(@"还没有添加买手店联系信息，点击右上角编辑",nil)],kDefaultBorderColor,@"noinfo_icon");
    ...
    [_buyerDetailModel.userContactInfos enumerateObjectsUsingBlock:^(NSDictionary *obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [_buyerDetailModel.userSocialInfos enumerateObjectsUsingBlock:^(NSDictionary *obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
-(void)PrepareData
{
    ...
    [_buyerDetailModel.userContactInfos enumerateObjectsUsingBlock:^(NSDictionary *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        {
            haveValue = YES;
            auth = [[obj objectForKey:@"auth"] integerValue];
        }
        ...
        if(haveValue)
        {
            ...
            {
                //合作可见
                if([_buyerDetailModel.connectStatus integerValue] == 1)
                {
                    //已经合作了
                    isVisible = YES;
                }else
    ...
    if(_contactTempArr.count)
    {
        ...
        for (int i = 0; i < _contactTempArr.count - 1; i++) {
            ...
            for (int j = 0; j < _contactTempArr.count - 1 - i; j++) {
                ...
                NSInteger nextidx=nextcontactType==0?0:nextcontactType==1?2:nextcontactType==2?3:nextcontactType==3?4:nextcontactType==4?2:-1;
